...
namespace mystd {
	...
	public:
		...
		template<std::size_t N>
		constexpr auto get() const -> const select_t<N, Types...>&//std::add_lvalue_reference_t<select_t<N, Types...>>
		{
			return baseValue<N>();
		}
...
int main()
{
	...
	mystd::Tuple bb(aa);
	//bb.set<0>(1000);
	//mystd::Tuple<double&, int&, char&, long, const char*> bb(aa);
	//std::remove_cv_t<decltype(aa)> bb(aa);
	mystd::Tuple<> ttt;
