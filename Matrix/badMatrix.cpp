...
private:
	...
	void initStrides() noexcept
	{
		...
		for (auto beg = extents.cbegin(), dest = strides.begin(); beg != extents.cend(); ++beg, ++dest)
			*dest = temp /= *beg;
	}
...
public:
	...
	MatrixSlice(std::size_t s, const array &exts)
		: extents(exts), size(initSize()), start(s) { initStrides(); }
	MatrixSlice(std::size_t s, const array &exts, const array &strs)
		: extents(exts), strides(strs), size(initSize()), start(s) { }

	MatrixSlice(const array &stas, const array &exts, const array strs, const MatrixSlice &other) :
		start(other.start)
	{
		initExtsStrs(stas, exts, strs, other.extents, other.strides);
		size = initSize();
		initStrides();
		strides = other.strides;
		if (checkArgs(stas, extents, other.extents))
			throw std::out_of_range("MatrixSlice stas exts: out of range");
		if (checkStrides(other.size))
			throw std::out_of_range("MatrixSlice strides: out of range");
	}

	template<typename... Exts, typename = std::enable_if_t<IsAllIntegral<Exts...>>>
	explicit MatrixSlice(Exts... exts)  : extents{exts...}, size(initSize()), start(0) { initStrides(); }
...
private:
	...
	void initExtsStrs(const array &stas, const array &exts, /* 待更新 */const array &strs, const array &oldExts, const array &oldStrs)
	{
	...
	bool checkStrides(std::size_t oldSize) const noexcept
	{
		// 待更新
		return false;
	}
...
{
public:
	std::ptrdiff_t operator-(const MatrixIterBase &rhs) const noexcept {
		return cur - rhs.cur;
	}

protected:
	MatrixIterBase() : cur(0), slice(nullptr) { }
	MatrixIterBase(std::size_t s) : cur(s), slice(nullptr) { }
	MatrixIterBase(const MatrixSlice<Dime> &sle) : cur(0), slice(&sle) { }
	MatrixIterBase(const MatrixSlice<Dime> &sle, std::size_t s) : cur(s), slice(&sle) { }

	using array = std::array<std::ptrdiff_t, Dime>;

	void increased()
	{
		const auto &extents = slice->getExtents();
		for (std::size_t i = Dime; --i;)
			if (indexes[i] >= extents[i])
			{
				indexes[i - 1] += indexes[i] / extents[i];
				indexes[i] %= extents[i];
			}
	}
	void decrement()
...
public:
	...
	MatrixIterator() : elems(nullptr) { }
	MatrixIterator(std::size_t s) : Base(s), elems(nullptr) { }
	MatrixIterator(const MatrixSlice<Dime> &sle, Type *elms, std::size_t s) : Base(sle, s), elems(elms) { }
	MatrixIterator(const MatrixSlice<Dime> &sle, Type *elms) : Base(sle), elems(elms) { }
	...
	MatrixIterator& operator+=(std::ptrdiff_t n)
	{
		if (n < 0)
			*this -= std::abs(n);
		else
	...
	MatrixIterator operator+(std::ptrdiff_t n) const
	{
	...
	std::ptrdiff_t current() const noexcept { return cur; }
...
public:
	...
	ConstMatrixIterator(const ConstMatrixIterator&) = default;
	ConstMatrixIterator(ConstMatrixIterator&&) = default;
	ConstMatrixIterator& operator=(const ConstMatrixIterator&) = default;
	ConstMatrixIterator& operator=(ConstMatrixIterator&&) = default;

	ConstMatrixIterator() : elems(nullptr) { }
	ConstMatrixIterator(std::size_t s) : Base(s), elems(nullptr) { }
	ConstMatrixIterator(const MatrixSlice<Dime> &sle, const Type *elms) : Base(sle), elems(elms) { }
	...
	ConstMatrixIterator& operator+=(std::ptrdiff_t n)
	{
		if (n < 0)
			*this -= std::abs(n);
		else
	...
	ConstMatrixIterator operator+(std::ptrdiff_t n) const
	{
...
protected:
	...
	MatrixBase(Args&&... args) : slice(std::forward<Args>(Args)...) { }
...
public:
	...
	explicit Matrix(Exts... exts) : Base(std::size_t(exts)...), elems(slice.size) { }
	template<typename... Exts, typename = std::enable_if_t<sizeof...(Exts) == Dime && !IsAllIntegral<Exts...>>>
	explicit Matrix(Type val, Exts... exts) : Base(std::size_t(exts)...), elems(slice.size, val) { }

	Matrix(const MatrixInitList<Type, Dime> &init) :
		Base(0, impl::deriveExtents<Type, Dime>(init)), elems(MatrixParser<Type, Dime>(init, this->slice.getExtents())) { }

	Matrix& operator=(const MatrixInitList<Type, Dime> &init)
	{
		this->slice = MatrixSlice<Dime>(0, impl::deriveExtents<Type, Dime>(init));
	...
	{
		static_assert(std::is_convertible_v<T, Type>);
		this->slice = MatrixSlice<Dime>(0, init.getExtents());
		elems = init;
		return *this;
	}
	...
	{
		static_assert(std::is_convertible_v<T, Type>);
		this->slice = MatrixSlice<Dime>(0, init.slice.extents);
		elems.assign(init.begin(), init.end());
		return *this;
	}

	template<typename T, typename = std::enable_if_t<std::is_convertible_v<T, Type>>>
	Matrix(const Matrix<T, Dime> &rhs) : Base(rhs), elems(rhs.elems.data(), rhs.elems.data() + rhs.elems.size()) { }

	template<typename T, typename = std::enable_if_t<std::is_convertible_v<T, Type>>>
	Matrix(const MatrixParser<T, Dime> &psr) : Base(0, psr.getExtents()), elems(psr) { }

	Matrix(MatrixParser<Type, Dime> &&psr) : Base(0, psr.getExtents()), elems(std::move(psr)) { }

	std::size_t size() const noexcept { return this->slice.size; }
	std::size_t extent(std::size_t n = 0) const noexcept { return this->slice.extent(n); }
	constexpr static std::size_t dimensions() { return Dime; }
	std::size_t rows() const noexcept { return this->slice.rows(); }
	std::size_t cols() const noexcept { return this->slice.cols(); }
...
#if 0
	iterator begin() noexcept { return { slice, elems.data() }; }
	iterator end() noexcept { return { slice, elems.data(), slice.size }; }
	const_iterator begin() const noexcept { return { slice, elems.data() }; }
	const_iterator end() const noexcept { return { slice, elems.data(), slice.size }; }
	const_iterator cbegin() const noexcept { return { slice, elems.data() }; }
	const_iterator cend() const noexcept { return { slice, elems.data(), slice.size }; }
#endif
...
{
	friend class Matrix<Type, Dime>;
	friend class Matrix<std::reference_wrapper<const Type>, Dime>;
	friend class MatrixBase<Type, Dime>;
	using Base = MatrixBase<Type, Dime>;
...
public:
	...
	iterator end() noexcept { return { slice.size }; }
	const_iterator begin() const noexcept { return { slice, elems }; }
	const_iterator end() const noexcept { return { slice.size }; }
	const_iterator cbegin() const noexcept { return { slice, elems }; }
	const_iterator cend() const noexcept { return { slice.size }; }
...
{
	friend class Matrix<Type, Dime>;
	friend class Matrix<std::reference_wrapper<Type>, Dime>;
	friend class MatrixBase<Type, Dime>;
	using Base = MatrixBase<Type, Dime>;
public:
	using Base::DimeReduction;
...
namespace std {
template<typename T, std::size_t D>
struct iterator_traits<mystd::matrix::MatrixIterator<T, D>>
{
	using size_type = typename mystd::matrix::MatrixIterator<T, D>::size_type;
	using difference_type = typename mystd::matrix::MatrixIterator<T, D>::difference_type;
	using value_type = typename mystd::matrix::MatrixIterator<T, D>::value_type;
	using pointer = typename mystd::matrix::MatrixIterator<T, D>::pointer;
	using const_pointer = typename mystd::matrix::MatrixIterator<T, D>::const_pointer;
	using reference = typename mystd::matrix::MatrixIterator<T, D>::reference;
	using const_reference = typename mystd::matrix::MatrixIterator<T, D>::const_reference;
	using iterator_category = typename mystd::matrix::MatrixIterator<T, D>::iterator_category;
};
template<typename T, std::size_t D>
struct iterator_traits<mystd::matrix::ConstMatrixIterator<T, D>>
{
	using size_type = typename mystd::matrix::ConstMatrixIterator<T, D>::size_type;
	using difference_type = typename mystd::matrix::ConstMatrixIterator<T, D>::difference_type;
	using value_type = typename mystd::matrix::ConstMatrixIterator<T, D>::value_type;
	using pointer = typename mystd::matrix::ConstMatrixIterator<T, D>::pointer;
	using const_pointer = typename mystd::matrix::ConstMatrixIterator<T, D>::const_pointer;
	using reference = typename mystd::matrix::ConstMatrixIterator<T, D>::reference;
	using const_reference = typename mystd::matrix::ConstMatrixIterator<T, D>::const_reference;
	using iterator_category = typename mystd::matrix::ConstMatrixIterator<T, D>::iterator_category;
};
}
...
int main()
{
	...
	auto mm = m(Slice(1), Slice(1, 1), Slice(1,1), Slice(0));
	MatrixIterator<int, 4>::pointer p = nullptr;
	std::cout << std::is_pointer<MatrixIterator<int, 4>::pointer>::value << std::endl;
	//auto b = mm.begin(); auto e = mm.end();
	//auto c = b;
	std::cout << m << std::endl;
	//m[0] = m[1];
